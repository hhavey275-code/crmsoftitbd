...
export function AdminBanks() {
  ...
  const assignMutation = useMutation({
    mutationFn: async () => {
      const { error } = await (supabase as any).from("client_banks").insert({
        user_id: selectedClient,
        bank_account_id: showAssign,
      });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("Bank assigned to client!");
      queryClient.invalidateQueries({ queryKey: ["admin-client-banks"] });
      setShowAssign(null);
      setSelectedClient("");
    },
    onError: (e: any) => toast.error(e.message),
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">All Banks</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                ...
                <TableRow key={b.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button size="sm" variant="ghost" className="hover:text-destructive" onClick={() => deleteMutation.mutate(b.id)}>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Client Bank Assignments</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
