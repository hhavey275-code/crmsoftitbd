    ...
    const promises = (accounts ?? []).map(async (account: any) => {
      ...
      try {
        ...
        const fetchPromises: Promise<Response>[] = [
          fetch(todayUrl),
          fetch(`https://graph.facebook.com/v24.0/${actId}?fields=balance,amount_spent,funding_source_details,all_payment_methods{pm_credit_card_type,display_string,funding_source_id}&access_token=${accessToken}`),
